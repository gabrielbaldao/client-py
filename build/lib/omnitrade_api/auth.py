...
class Auth(object):
    ...
    def signed_challenge(self, challenge):
        signature = 'signature'#hashlib.pbkdf2_hmac('sha256', self.secret_key,"{access_key}{challenge}".format(access_key = self.access_key, challenge = challenge), 100000 )
    ...
    def __sign(self, verb, path, params):
        return 'signature'#hashlib.pbkdf2_hmac('sha256', self.secret_key, self.__payload(verb, path, params), 100000)
